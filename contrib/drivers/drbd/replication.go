...
func (r *ReplicationDriver) CreateReplication(opt *pb.CreateReplicationOpts) (*model.ReplicationSpec, error) {
	...
	// TODO(rck): That needs a change, we now have this information in the data:
	...
	// this covers also the empty case
	if myHost.Name == peerHost.Name || myHost.ID == peerHost.ID || myHost.IP == peerHost.IP {
	...
	for _, volData := range opt.GetVolumeDataList() {
		...
		if val, ok := data[portKey(primaryVolID)]; ok {
			return nil, fmt.Errorf("Primary Volume ID (%s), already has a port Key (%s)", val)
		}
		if val, ok := data[portKey(secondaryVolID)]; ok {
			return nil, fmt.Errorf("Secondary Volume ID (%s), already has a port Key (%s)", val)
		}
		if val, ok := data[minorKey(primaryVolID)]; ok {
			return nil, fmt.Errorf("Primary Volume ID (%s), already has a minor Key (%s)", val)
		}
		if val, ok := data[minorKey(secondaryVolID)]; ok {
			return nil, fmt.Errorf("Secondary Volume ID (%s), already has a minor Key (%s)", val)
		}
	...
	additionalPrimaryData := map[string]string{
		portKey(primaryVolID):  strconv.FormatInt(10, port),
		minorKey(primaryVolID): strconv.FormatInt(10, minor),
	}

	additionalSecondaryData := map[string]string{
		portKey(secondaryVolID):  strconv.FormatInt(10, port),
		minorKey(secondaryVolID): strconv.FormatInt(10, minor),
	}
